...
public:
     ...
     * @remarks Console logging will only be enabled if set in the configuration.
     */
    static void Initialize()
    {
        ...
        else
        {
            ...
            const auto logger = spdlog::basic_logger_mt("default", LogFilePath, true);
            set_default_logger(logger);
...
private:
    /**
     * Creates a file path string for a new log file, where the name contains a timestamp for when the game was launched.
    ...
    static std::string CreateLogPath()
    {
        ...
        return fmt::format(R"({}\Flagrum\logs\game_{:%Y%m%d%H%M%S}.log)", path, fmt::localtime(now));
    }

    /**
     * Opens a console and redirects standard output and standard error output to it.
     */
