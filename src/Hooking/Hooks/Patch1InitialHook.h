...
namespace Hooks
{
     ...
     * @endcode
     * @remarks This is one of two hooks that adds a patch archive to the asset manager.
     *          This one is hooked earlier in the game's initialization process, as calling
     *          it later prevents it from acting on assets that are loaded very early on.
     */
    class Patch1InitialHook final : public FunctionHook<0x4FBA60, 0x4CE960, int64_t, void*>
    {
    private:
        using AcquireAsset_t = void*(__fastcall*)(void* pAssetManager, LmAssetID*, uint64_t, int64_t);
    ...
    protected:
         ...
         * @remarks This detour will add patch/patch1_initial/patchindex.earc to the asset manager on first call.
         */
        int64_t Detour(void* pApplicationBase) override
        {
            std::call_once(hasApplied_, []
            {
                ...
                CPPTRACE_TRY
                {
                    ...
                    auto patchIndexId = LmAssetID::Create("data://patch/patch1_initial/patchindex.ebex@");
                    SQEX::Luminous::AssetManager::LmFileList::AddPatchIndexEarc(&patchIndexId);

                    // This call is needed to finish this off, but it's not 100% clear what it does
                    const auto assetManager = SQEX::Luminous::AssetManager::LmGetAssetManager();
                    const auto assetManagerBaseAddress = *static_cast<uint64_t*>(assetManager);
                    const auto acquireAsset = *reinterpret_cast<AcquireAsset_t*>(assetManagerBaseAddress + 8);
                    acquireAsset(assetManager, &patchIndexId, 0, 2);
                }
                CPPTRACE_CATCH(...)
                {
                    Exception::Fatal("Failed to add patch1_initial to asset manager");
                }
