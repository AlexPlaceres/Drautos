...
namespace SQEX::Luminous::AssetManager
{
    ...
    public:
         ...
         * @remarks This is a recreation of the function from debug as it doesn't appear to exist in release.
         */
        static void AddPatchIndexEarc(LmAssetID* lmAssetID)
        {
            // Grab the patch index list
            const auto lmFileList = reinterpret_cast<char*>(REBASE(0x5ADFB90, 0x4CE7C18));
            const auto pPatchIndexEarcList = reinterpret_cast<std::vector<uint64_t>*>(lmFileList + 0x138);

            // For some accursed reason, the vector crashes on push_back on the debug build if we don't create a new one
