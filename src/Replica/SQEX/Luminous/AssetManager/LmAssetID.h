...
namespace SQEX::Luminous::AssetManager
{
    ...
    public:
        /**
         * The raw value of the asset ID. This is a combined hash of the URI and type (file extension).
         */
        uint64_t fullHash_;
         ...
         * @remarks If the URI hash has already been computed, the stored value is returned,
         *          otherwise it will be computed, stored,and then returned.
         */
        static uint64_t GetNameHash(LmAssetID* pAssetId, uint64_t* result)
        {
            ...
            *result = uri_hash;
            ...
            if (uri_hash == 0)
            {
                ...
                const auto new_uri_hash = GenerateNameHash(&name_hash_result, &pAssetId->m_path);
                *result = new_uri_hash;
        ...
        /**
         * Generates the hash of the given URI, excluding the file extension (type) component.
         * @param result The location to store the resulting hash, same as the return value.
         * @param path The URI to hash.
         * @return The computed hash.
         */
        static uint64_t GenerateNameHash(uint64_t* result, const std::string* path)
        {
            *result = Core::Fnv1a64Lower(path->data(), 0x14650FB0739D0383) & 0xFFFFFFFFFFF;
            return *result;
        }
